...
async function runAllTests() {
  ...
  if (!projectId || !apiKey || !databaseId || !attendeesCollectionId) {
  ...
  await runTest('Test 3: Bulk Update Attendees (Atomic)', async () => {
    ...
    const existingDocs = await Promise.all(
      createdAttendeeIds.slice(0, 5).map(id =>
        databases.getDocument(databaseId!, attendeesCollectionId!, id)
      )
    );

    // Prepare updates
    const rows = existingDocs.map(doc => {
      const { $permissions, $createdAt, $updatedAt, $collectionId, $databaseId, ...docData } = doc as any;
      return {
        ...docData,
  ...
  await runTest('Test 4: Bulk Delete Attendees (Atomic)', async () => {
    ...
    for (const id of idsToDelete) {
      try {
        await databases.getDocument(databaseId!, attendeesCollectionId!, id);
  ...
  await runTest('Test 5: Custom Fields Reorder (Atomic)', async () => {
    ...
    if (!eventSettingsCollectionId) {
      throw new Error('NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_COLLECTION_ID not set');
    }
    ...
    const existingFields = await Promise.all(
      createdCustomFieldIds.map(id =>
        databases.getDocument(databaseId!, customFieldsCollectionId!, id)
      )
    );

    // Reverse the order
    const rows = existingFields.map((field, index) => {
      const { $permissions, $createdAt, $updatedAt, $collectionId, $databaseId, ...fieldData } = field as any;
      return {
        ...fieldData,
  ...
  await runTest('Test 8: Audit Log Creation', async () => {
    ...
    const log = await databases.getDocument(databaseId!, logsCollectionId!, logId);
