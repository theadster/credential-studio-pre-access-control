...
async function verifyBooleanFieldFormat() {
  ...
  try {
    ...
    const stats = {
      ...
      emptyValues: 0,
      malformedJson: 0       // JSON parse failures
    };
    ...
    const malformedAttendees: Array<{
      attendeeId: string;
      attendeeName: string;
      error: string;
    }> = [];
    ...
    for (const attendee of allAttendees) {
      ...
      if (typeof attendee.customFieldValues === 'string') {
        try {
          ...
          // Record JSON parse failures for diagnostics
          stats.malformedJson++;
          malformedAttendees.push({
            attendeeId: attendee.$id,
            attendeeName: `${attendee.firstName} ${attendee.lastName}`,
            error: e.message
          });
          console.warn(`⚠️  JSON parse error for attendee ${attendee.$id}: ${e.message}`);
          continue;
        }
    ...
    console.log(`❌ Malformed JSON: ${stats.malformedJson}`);
    console.log('='.repeat(60));
    ...
    // Show malformed JSON records
    if (malformedAttendees.length > 0) {
      console.log('\n❌ MALFORMED JSON RECORDS (parse failures):');
      console.log('-'.repeat(60));
      malformedAttendees.forEach(record => {
        console.log(`  ${record.attendeeName} (ID: ${record.attendeeId})`);
        console.log(`    Error: ${record.error}`);
      });
      console.log('\n⚠️  These records have invalid JSON and may need manual review or re-migration.');
    }

    // Final status
    console.log('\n' + '='.repeat(60));
    if (stats.corruptedFormat === 0 && stats.invalidFormat === 0 && stats.malformedJson === 0) {
    ...
    } else if (stats.malformedJson > 0) {
      console.log('❌ DATA INTEGRITY ISSUES!');
      console.log(`   ${stats.malformedJson} records have malformed JSON that cannot be parsed.`);
      console.log('   These records may need manual review or re-migration.');
    } else {
