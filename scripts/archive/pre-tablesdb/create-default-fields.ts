 ...
 * 
 * Run with: npx tsx scripts/archive/pre-tablesdb/create-default-fields.ts
 */
...
async function createDefaultFields() {
  ...
  try {
    ...
    // Compute base order once from existing fields to avoid collisions
    const baseOrder = existingFields.documents.reduce(
    ...
    if (!credentialTypeExists) {
      ...
      await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          required: false,
          order: baseOrder + 1, // Use baseOrder + 1 for first new field
          showOnMainPage: true,
          version: 0
        }
      );
    ...
    if (!notesExists) {
      ...
      await databases.createDocument(
        ...
        ID.unique(),
        {
          ...
          fieldOptions: null,
          required: false,
          order: baseOrder + 2, // Use baseOrder + 2 for second new field
          showOnMainPage: true,
          version: 0
        }
      );
