 ...
 * 
 * Usage:
 * npx tsx scripts/archive/pre-tablesdb/migrate-operator-fields.ts
 */
...
async function migrateOperatorFields() {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_APPWRITE_ATTENDEES_TABLE_ID) {
      throw new Error('NEXT_PUBLIC_APPWRITE_ATTENDEES_TABLE_ID is not set');
    }
    ...
    while (hasMore) {
      ...
      for (const attendee of attendees) {
        try {
          ...
          if (needsUpdate) {
            // Build sparse update object - only include fields that are null
            const updates: any = {};
            
            if (attendee.credentialCount == null) {
              updates.credentialCount = credentialCount;
            }
            if (attendee.photoUploadCount == null) {
              updates.photoUploadCount = photoUploadCount;
            }
            if (attendee.viewCount == null) {
              updates.viewCount = viewCount;
            }
            if (attendee.lastCredentialGenerated == null) {
              updates.lastCredentialGenerated = lastCredentialGenerated;
            }
            if (attendee.lastPhotoUploaded == null) {
              updates.lastPhotoUploaded = lastPhotoUploaded;
            }
