  ...
  try {
    ...
    for (const attendee of allAttendees) {
      ...
      try {
        ...
        if (Array.isArray(parsed)) {
          ...
          if (!dryRun && convertedData) {
            try {
              ...
              result.success = true;
              migratedCount++;
              console.log(`✅ Migrated: attendeeId ${attendee.$id} — Fields: ${result.fieldCount}`);
            } catch (error: any) {
            ...
            console.log(`🔍 Would migrate: attendeeId ${attendee.$id} — Fields: ${result.fieldCount}`);
        ...
        invalidFormatCount++;
        console.error(`❌ Parse error for attendeeId ${attendee.$id}:`, error.message);
      }
    }
    ...
    if (results.length > 0) {
      ...
      results.forEach((result, index) => {
        console.log(`${index + 1}. attendeeId ${result.attendeeId}`);
