...
async function main() {
  try {
    ...
    if (!process.env.NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_TABLE_ID) {
      throw new Error('NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_TABLE_ID is not set');
    }
    ...
    const collectionId = process.env.NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_TABLE_ID!;
