 ...
 * 
 * Usage:
 * npx tsx scripts/archive/pre-tablesdb/cleanup-help-center.ts
 */
...
async function deleteHelpCenterAttributes() {
    try {
        ...
        // Map TABLE_ID env vars to collection IDs (legacy support)
        // These env vars use TABLE_ID naming but getCollection() expects collection IDs
        const collectionsToCheck = [
            process.env.NEXT_PUBLIC_APPWRITE_USERS_COLLECTION_ID || process.env.NEXT_PUBLIC_APPWRITE_USERS_TABLE_ID,
            process.env.NEXT_PUBLIC_APPWRITE_ROLES_COLLECTION_ID || process.env.NEXT_PUBLIC_APPWRITE_ROLES_TABLE_ID,
            process.env.NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_COLLECTION_ID || process.env.NEXT_PUBLIC_APPWRITE_EVENT_SETTINGS_TABLE_ID,
        ].filter(Boolean);
...
async function deleteHelpCenterPermissions() {
    try {
        ...
        const roles = await databases.listDocuments(DATABASE_ID, rolesTableId);

        let updatedCount = 0;
        for (const role of roles.documents) {
            try {
                // Safely parse permissions - handle string, object, or null
                let permissions: any;
                if (typeof role.permissions === 'string') {
                    try {
                        permissions = JSON.parse(role.permissions);
                    } catch (parseError: any) {
                        console.warn(`  ⚠ Warning: Malformed JSON in role "${role.name}" (ID: ${role.$id}). Using empty permissions object.`);
                        permissions = {};
                    }
