...
async function diagnoseCustomFieldSearch() {
  ...
  if (!APPWRITE_ENDPOINT) missingVars.push('NEXT_PUBLIC_APPWRITE_ENDPOINT');
  if (!APPWRITE_PROJECT_ID) missingVars.push('NEXT_PUBLIC_APPWRITE_PROJECT_ID');
  if (!APPWRITE_API_KEY) missingVars.push('APPWRITE_API_KEY');
  if (!DATABASE_ID) missingVars.push('NEXT_PUBLIC_APPWRITE_DATABASE_ID');
  if (!CUSTOM_FIELDS_TABLE_ID) missingVars.push('NEXT_PUBLIC_APPWRITE_CUSTOM_FIELDS_TABLE_ID');
  if (!ATTENDEES_TABLE_ID) missingVars.push('NEXT_PUBLIC_APPWRITE_ATTENDEES_TABLE_ID');
