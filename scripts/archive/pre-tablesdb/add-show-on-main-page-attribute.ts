 ...
 * 
 * Usage:
 * npx tsx scripts/archive/pre-tablesdb/add-show-on-main-page-attribute.ts
 */
...
async function main() {
  try {
    ...
    if (attributeCreated) {
      ...
      // Fail fast if attribute didn't become available
      if (!attributeAvailable) {
        console.error('✗ showOnMainPage attribute did not become available within the timeout period');
